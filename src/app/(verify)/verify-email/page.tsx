...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      ...
      try {
        const result = await api.get(`/email-verification/verify-email?token=${token}`)
      ...
      <div className="w-full bg-white rounded-lg px-5 mb-10 flex flex-col justify-center items-center gap-4 py-12">
        ...
        <div className="text-center">
          ...
          <p className="text-neutr-500 font-araboto">
            {error || (verifying
              ? 'Estamos verificando seu email para ativar o suporte do seu produto.'
              : 'Seu email foi verificado com sucesso. Essa página será fechada automaticamente em alguns segundos.')}
          </p>
        </div>
      </div>
    </Container>
